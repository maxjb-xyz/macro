...
| Immutable images | Runtime containers use pinned release images, not host-built dev bundles or bind-mounted binaries. Rollback is changing image tags, not editing host artifacts. | `docker/selfhost/compose.published.yml` replaces host-built dev images with per-service `ghcr.io` release images for all 13 Rust services + proxy. JS/worker services (sync, lexical, ai_editing_worker, analytics_proxy, websocket_service) still build on-host. | Keep `compose.published.yml` in sync with CI's image list and publish release images for the remaining JS/worker services. Pin by SHA; document specialized-image exceptions. | Rendered config for production overlay has no dev `build:` stanzas for released services; image tags are immutable SHAs; rollback test starts previous tag set. |
