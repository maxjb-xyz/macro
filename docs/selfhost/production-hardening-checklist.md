...
- 9 services render without a Compose healthcheck:
  `ai_editing_worker`, `analytics_proxy`, `document_upload_finalizer`,
  `email_pubsub_workers`, `kafka_topics`, `mailpit`, `postgres_bootstrap`,
  `static_file_cdn`, and `websocket_service`.
...
| Restart policies | Every long-running production service restarts after process crash and host reboot, with graceful stop windows for stateful services and workers. One-shot bootstrap jobs do not restart forever. | `docker/docker-compose.self-host.yml` adds `restart: unless-stopped` only to stateful/local-emulated infrastructure. | Extend the self-host overlay with `restart: unless-stopped` for long-running app, worker, JS, and edge services. Keep one-shot jobs such as `postgres_bootstrap` and `kafka_topics` as `restart: "no"` or omitted. Preserve `stop_grace_period` for stateful stores and add worker-specific grace periods where in-flight jobs need shutdown time. | `docker compose config --format json` shows restart policy on every long-running service and no infinite restart loop on one-shot jobs; `docker restart`/host reboot smoke passes. |
| Health checks | Every service that receives traffic or gates dependency startup has a meaningful healthcheck; workers have a liveness or dependency probe where practical. | Most HTTP services have checks; workers, Mailpit, static CDN, websocket service, and bootstrap jobs do not. | Add healthchecks for `mailpit`, `static_file_cdn`, `websocket_service`, and JS/worker services that expose health endpoints. For pure workers without an endpoint, either add a lightweight health endpoint in the image or document why the process liveness plus queue smoke test is the check. Avoid making one-shot bootstrap jobs long-running only to satisfy healthcheck shape. | `docker compose ps` reports healthy services after boot; smoke plan covers workers by exercising queue-backed behavior. |
| Resource limits | Stateful stores and application services have explicit CPU/memory budgets sized for the host so one component cannot exhaust the node. | No rendered service has Compose `deploy.resources` reservations/limits. | Add an operator sizing overlay with `deploy.resources.limits` and, where supported by the target Compose runtime, reservations for Postgres, Redis, Kafka, OpenSearch, FusionAuth, Rust services, JS workers, and local emulators. Keep defaults conservative and document that operators must tune for dataset size and concurrent users. | Rendered config contains resources for all long-running services; load/smoke test shows no OOM loops; host monitoring confirms headroom. |
| Log retention | Container logs rotate locally and important logs are exported off-host before rotation. | No rendered service has `logging:` policy. | Add a Compose `logging` block, usually `driver: json-file` with `max-size`/`max-file`, or the operator's centralized log driver. State retention target and redaction policy. Avoid unlimited Docker JSON logs on production hosts. | `docker inspect` shows log rotation on each service; log volume cannot fill disk during a soak run; off-host logs contain startup, health, auth, worker, and error events. |
...
| Immutable images | Runtime containers use pinned release images, not host-built dev bundles or bind-mounted binaries. Rollback is changing image tags, not editing host artifacts. | Base Compose uses local development builds; prototype release overlay covers `authentication-service` and `static_file_service` only. | Promote `docker/selfhost/compose.release-images.example.yml` into a full release overlay after CI publishes per-service images. Use tags by service and git SHA/stable channel, remove inherited `build:` and dev `command:` fields, and document exceptions for services with specialized images. | Rendered config for production overlay has no dev `build:` stanzas for released services; image tags are immutable SHAs; rollback test starts previous tag set. |
| Update and rollback path | Operators can take a backup, apply new Compose/env/image versions, run smoke checks, and roll back to the previous known-good tag/env/backup. | Backup/restore and release-image docs define the pieces; no single update runbook exists yet. | Add an update runbook that records current commit, Compose files, env checksum, image tags, and backup ID before changes. Use pinned images and a checked-in overlay set so rollback is `git checkout`/env restore/image tag restore plus database restore when migrations are not backward compatible. | Practice upgrade and rollback on a non-production host; record RPO/RTO, backup ID, image tag set, and smoke results. |
...
Implement hardening as additive overlays under `docker/selfhost/` so local/dev Compose
stays easy to rebase:

1. `docker/docker-compose.self-host.yml` keeps baseline lifecycle behavior for the
   single-host appliance.
2. A future production hardening overlay should add:
   - restart policies for all long-running services;
   - healthchecks for missing HTTP/local-emulated services;
   - logging rotation defaults;
   - conservative resource limits/reservations;
   - optional labels/env needed by the chosen reverse proxy and monitoring stack.
3. `docker/selfhost/compose.release-images.example.yml` should graduate from
   example to production overlay only after CI publishes images for every required
   service.
4. Reverse-proxy configuration can live outside Compose if the operator already
   owns host ingress, but the deployment runbook must still record its routing,
   TLS, timeout, and WebSocket settings.
