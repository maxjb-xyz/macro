...
| `FUSIONAUTH_PUBLIC_URL` | `public URL` | Browser-reachable FusionAuth origin for SSO login/logout. A **separate host** — the `auth.` subdomain — routed to FusionAuth's published port `9011` (e.g. a Cloudflare Tunnel route `auth.example.com` → `http://localhost:9011`). Also drives FusionAuth's `FUSIONAUTH_APP_URL` (`fusionauth-app.url`). Without it, Google/GitHub SSO and logout redirect to the internal `fusionauth:9011`/`localhost` and fail. **Must be a single-level subdomain** (one label below the zone apex): Cloudflare's free Universal SSL wildcard `*.example.com` covers `app.example.com` but NOT `auth.app.example.com`, which fails HTTPS with a TLS handshake error ("uses an unsupported protocol"). The frontend derives its logout URL as `auth.<registrable-domain>`, so keep this in lockstep with `BASE_URL`. |
